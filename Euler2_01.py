...
theta = np.maximum(np.abs(kx + 2*np.pi*np.floor((-np.pi-kx)/(2*np.pi) + 1)),
                   np.abs(ky + 2*np.pi*np.floor((-np.pi-ky)/(2*np.pi) + 1)))
...
V2 = np.array([
    ...
    [-np.sin(theta),0*kx,np.cos(theta)]
])
V2 = np.transpose(V2,(2,3,0,1))

# Checking the euler class of the final structure
vectors = gauge_fix_grid(V2)
