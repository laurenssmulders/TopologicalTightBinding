...
if slicing:
    ...
    for i in range(100):
        ...
        plt.ylabel('$E / J$')
        plt.savefig('figures/square/SP1/driven/bandstructures/SP1_driven_Ax_w_dA_dC_{A_x}_{omega}_{delta_A}_{delta_C}_sliced/xslices/SP1_driven_Ax_w_dA_dC_{A_x}_{omega}_{delta_A}_{delta_C}_xslice{i}'.format(A_x=A_x, omega=omega, delta_A=delta_A, delta_C=delta_C, i=i))
        plt.close()
    ...
    for i in range(100):
        ...
        plt.xticks([0,np.pi / 2, np.pi, 3 * np.pi / 2, 2 * np.pi],
                   [0,'$\pi / 2$', '$\pi$', '$3\pi / 2$', '$2\pi$'])
        plt.ylabel('$E / J$')
        plt.savefig('figures/square/SP1/driven/bandstructures/SP1_driven_Ax_w_dA_dC_{A_x}_{omega}_{delta_A}_{delta_C}_sliced/yslices/SP1_driven_Ax_w_dA_dC_{A_x}_{omega}_{delta_A}_{delta_C}_yslice{i}'.format(A_x=A_x, omega=omega, delta_A=delta_A, delta_C=delta_C, i=i))
        plt.close()